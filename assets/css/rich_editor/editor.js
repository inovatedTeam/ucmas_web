					...
					reader.onload = (function(imageFile){
						return function(e){
							...
							var image = $('<img/>',{
								...
								$('#imageList').data('current', "test");//$(this).attr('src')
								});
                            $('#imageList').data('current', e.target.result);
			...
			var menuItems = { 'fonteffects': true,
						   ...
						   'insert_img': {  "modal": true,
											...
											"onSave":function(){
												...
												var targetURL  = $('#inputImageUrl').val();
												if(targetURL ==''){
													methods.showMessage.apply(editorObj,["imgErrMsg","Please enter url"]);
													return false;
												}												
												if(!targetURL.match(urlPattern)){
													methods.showMessage.apply(editorObj,["imgErrMsg","Enter valid url"]);
													return false;
												}
												/*
												if(navigator.userAgent.match(/MSIE/i)){	
													console.log("insert - targetURL")
													console.log(targetURL)
													var targetLink='<img url="'+targetURL+'" />';
													methods.restoreSelection.apply(editorObj,[targetLink,'html']);
												}
												else{
												    methods.restoreSelection.apply(editorObj, [targetText]);																																		
												...
												*/
												if(targetURL){
													if(navigator.userAgent.match(/MSIE/i)){
														console.log("here1")
														var imageStr = '<img src="'+targetURL+'"/>'
														...
														var imageStr = '<img src="'+targetURL+'"/>'
														methods.restoreSelection.apply(this,[imageStr,'html'])
														// document.execCommand('insertimage', false, $('#inputImageUrl').val(''));
													}
												}
												else{
													console.log("here3")
